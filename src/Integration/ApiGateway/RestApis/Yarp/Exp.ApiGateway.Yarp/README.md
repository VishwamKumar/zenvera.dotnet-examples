# YARP REST API gateway

## Objective

Demonstrate ASP.NET Core reverse-proxy routing with YARP.
...
- Gateway/downstream service boundary.
...
Client → YARP route → configured cluster destination → downstream response.
...
Client → YARP route → configured cluster destination → downstream response. Validate both the successful path and the authentication, validation, connectivity, or not-found failure relevant to this example.
...
HTTP 6502; HTTPS 8502. The sample destination defaults to `https://localhost:7198/`; start or replace the downstream service address before testing.
...
```powershell
dotnet run --project src/Integration/ApiGateway/RestApis/Yarp/Exp.ApiGateway.Yarp --launch-profile https
```
...
- Run `dotnet build src/Integration/ApiGateway/RestApis/Yarp/Exp.ApiGateway.Yarp` before manual testing.
