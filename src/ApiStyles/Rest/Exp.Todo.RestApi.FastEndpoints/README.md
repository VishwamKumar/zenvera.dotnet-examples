# Todo FastEndpoints REST API

## Objective

Demonstrate class-based Todo endpoints using FastEndpoints.
...
- FastEndpoints request/response endpoint classes.
- Validation-ready endpoint organization.
...
HTTP request → FastEndpoints endpoint → shared Todo repository → SQLite.
...
HTTP request → FastEndpoints endpoint → shared Todo repository → SQLite. Validate both the successful path and the authentication, validation, connectivity, or not-found failure relevant to this example.
...
HTTP 5104; HTTPS 7104. SQLite is configured through `ConnectionStrings`.
...
```powershell
dotnet run --project src/ApiStyles/Rest/Exp.Todo.RestApi.FastEndpoints --launch-profile https
```
...
- Run `dotnet build src/ApiStyles/Rest/Exp.Todo.RestApi.FastEndpoints` before manual testing.
