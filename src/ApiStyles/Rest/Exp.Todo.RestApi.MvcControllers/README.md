# Todo MVC Controllers REST API

## Objective

Compare controller-based ASP.NET Core REST endpoints with the other Todo delivery styles.
...
- DTO mapping and conventional action results.
...
HTTP request → MVC controller action → shared Todo repository → SQLite.
...
HTTP request → MVC controller action → shared Todo repository → SQLite. Validate both the successful path and the authentication, validation, connectivity, or not-found failure relevant to this example.
...
HTTP 5102; HTTPS 7102. SQLite is configured through `ConnectionStrings`.
...
```powershell
dotnet run --project src/ApiStyles/Rest/Exp.Todo.RestApi.MvcControllers --launch-profile https
```
...
- Run `dotnet build src/ApiStyles/Rest/Exp.Todo.RestApi.MvcControllers` before manual testing.
