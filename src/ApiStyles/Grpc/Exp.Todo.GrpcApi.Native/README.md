# ToDoApp

## Maturity level

**Level 2 — Layered Example.** Delivery code is separated from shared Todo persistence. See the [catalog](../../../../docs/catalog.md) for repository-wide status and sequence.


## Description
A ToDo application with various API implementations.

# Authors

## Vishwa Kumar
- **Email:** vishwa@vishwa.me
- **GitHub:** [Vishwam](https://github.com/vishwamkumar)
- **LinkedIn:** [Vishwa Kumar](https://www.linkedin.com/in/vishwamohan)

Vishwa is the architect of the ToDoApp, responsible for the architecture and implementation of these features.

## License
This project is licensed under the MIT License - see the [root LICENSE](../../../../LICENSE) file for details.