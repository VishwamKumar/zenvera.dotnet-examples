# Todo native gRPC API

## Objective

Demonstrate strongly typed Todo RPC operations over native gRPC.
...
- Protocol Buffer service contracts.
...
gRPC client → generated service contract → service implementation → shared Todo repository → SQLite.
...
gRPC client → generated service contract → service implementation → shared Todo repository → SQLite. Validate both the successful path and the authentication, validation, connectivity, or not-found failure relevant to this example.
...
HTTP 5201; HTTPS 7201. Use an HTTP/2-capable gRPC client. SQLite is configured through `ConnectionStrings`.
...
```powershell
dotnet run --project src/ApiStyles/Grpc/Exp.Todo.GrpcApi.Native --launch-profile https
```
...
- Run `dotnet build src/ApiStyles/Grpc/Exp.Todo.GrpcApi.Native` before manual testing.
