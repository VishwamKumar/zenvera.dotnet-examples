...
Demonstrate contract-first-style Todo operations through CoreWCF SOAP hosting.
...
- CoreWCF service and operation contracts.
- SOAP data contracts and WSDL metadata.
...
SOAP client → CoreWCF endpoint → Todo service → shared Todo repository → SQLite.
...
SOAP client → CoreWCF endpoint → Todo service → shared Todo repository → SQLite. Validate both the successful path and the authentication, validation, connectivity, or not-found failure relevant to this example.
...
HTTP 5401; HTTPS 7401. Inspect the service metadata/WSDL before generating a client.
...
```powershell
dotnet run --project src/ApiStyles/Soap/Exp.Todo.SoapApi --launch-profile https
```
...
- Run `dotnet build src/ApiStyles/Soap/Exp.Todo.SoapApi` before manual testing.
