# Weather REST API with Serilog

## Objective

Demonstrate structured logging from an ASP.NET Core REST API.
...
- Structured controller events.
- Configurable SQL Server logging sink.
...
Client → Weather controller → structured log pipeline → configured sinks.
...
Client → Weather controller → structured log pipeline → configured sinks. Validate both the successful path and the authentication, validation, connectivity, or not-found failure relevant to this example.
...
HTTP 5114; HTTPS 6114. Configure SQL Server and sink settings outside source; authenticate to the private package feed for `Zenvera.Shared.Logging`.
...
```powershell
dotnet run --project src/Infrastructure/Logging/Serilog/Exp.Weather.RestApi.Serilog --launch-profile https
```
...
- Run `dotnet build src/Infrastructure/Logging/Serilog/Exp.Weather.RestApi.Serilog` before manual testing.
