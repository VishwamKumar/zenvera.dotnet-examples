# Blazor Server with Serilog

## Objective

Demonstrate structured logging from Blazor Server components.
...
- Blazor Server rendering and circuit events.
- Serilog integration through shared packages.
- MongoDB logging sink configuration.
...
Browser circuit → Blazor component → structured log pipeline → MongoDB/configured sinks.
...
Browser circuit → Blazor component → structured log pipeline → MongoDB/configured sinks. Validate both the successful path and the authentication, validation, connectivity, or not-found failure relevant to this example.
...
HTTP 5115; HTTPS 6115. Configure MongoDB and sink credentials outside source; private package-feed authentication is required.
...
```powershell
dotnet run --project src/Infrastructure/Logging/Serilog/Exp.Weather.BlazorServer.Serilog --launch-profile https
```
...
- Run `dotnet build src/Infrastructure/Logging/Serilog/Exp.Weather.BlazorServer.Serilog` before manual testing.
