# Weather REST API with RabbitMQ

## Objective

Demonstrate publishing Weather messages through RabbitMQ.
...
- Message publication through `Zenvera.Shared.Queuing`.
- REST-to-message boundary.
...
Client → Weather controller → RabbitMQ abstraction → exchange/queue broker.
...
Client → Weather controller → RabbitMQ abstraction → exchange/queue broker. Validate both the successful path and the authentication, validation, connectivity, or not-found failure relevant to this example.
...
HTTP 5112; HTTPS 6112. RabbitMQ uses port 5672 and management UI 15672. Configure credentials via environment variables and authenticate to the private package feed.
...
```powershell
dotnet run --project src/Infrastructure/Messaging/RabbitMq/Exp.Weather.RestApi.RabbitMq --launch-profile https
```
...
- Run `dotnet build src/Infrastructure/Messaging/RabbitMq/Exp.Weather.RestApi.RabbitMq` before manual testing.
