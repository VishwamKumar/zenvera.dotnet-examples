# Weather REST API with Redis caching

## Objective

Demonstrate cache-aside-style Weather responses through the shared caching package.
...
- Redis and in-memory provider selection.
- Cache configuration and key inspection.
- REST behavior around cached Weather data.
...
Client → Weather controller → caching abstraction → Redis/in-memory provider.
...
Client → Weather controller → caching abstraction → Redis/in-memory provider. Validate both the successful path and the authentication, validation, connectivity, or not-found failure relevant to this example.
...
HTTP 5111; HTTPS 6111. Redis defaults are under `RedisCacheSettings`; local Redis is available through `deploy/local/compose.yml`. Private package-feed authentication is required to restore.
...
```powershell
dotnet run --project src/Infrastructure/Caching/Redis/Exp.Weather.RestApi.RedisCaching --launch-profile https
```
...
- Run `dotnet build src/Infrastructure/Caching/Redis/Exp.Weather.RestApi.RedisCaching` before manual testing.
