# gRPC Todo Clean Architecture — CQRS with MediatR

## Objective

Show command/query separation using MediatR and compare it with the custom dispatcher.
...
- Pipeline-ready CQRS dispatch.
- Independent Domain/Application/Infrastructure/host projects and tests.
...
gRPC host → MediatR → command/query handler → Domain and persistence abstractions → Infrastructure.
...
gRPC host → MediatR → command/query handler → Domain and persistence abstractions → Infrastructure. Validate both the successful path and the authentication, validation, connectivity, or not-found failure relevant to this example.
...
HTTP 6403; HTTPS 8403. SQLite is configured through `ConnectionStrings`.
...
```powershell
dotnet run --project src/Architecture/CleanArchitecture/GrpcTodo/CqrsMediatR/src/Exp.Todo.GrpcApi --launch-profile https
```
...
- Run `dotnet build src/Architecture/CleanArchitecture/GrpcTodo/CqrsMediatR/src/Exp.Todo.GrpcApi` before manual testing.
