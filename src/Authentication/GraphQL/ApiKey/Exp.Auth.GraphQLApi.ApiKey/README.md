...
Show API-key validation in front of a GraphQL endpoint.
...
- GraphQL middleware authorization boundary.
...
GraphQL client → API-key middleware → resolver, or an authentication failure before execution.
...
GraphQL client → API-key middleware → resolver, or an authentication failure before execution. Validate both the successful path and the authentication, validation, connectivity, or not-found failure relevant to this example.
...
Use the launch-profile ports. Configure development keys through `ApiKeys` overrides and send `X-Api-Key`; never commit real credentials.
...
```powershell
dotnet run --project src/Authentication/GraphQL/ApiKey/Exp.Auth.GraphQLApi.ApiKey --launch-profile https
```
...
- Run `dotnet build src/Authentication/GraphQL/ApiKey/Exp.Auth.GraphQLApi.ApiKey` before manual testing.
