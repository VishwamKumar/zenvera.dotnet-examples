# GraphQL JWT bearer authentication

## Objective

Demonstrate JWT issuance and validation around GraphQL operations.
...
- JWT creation for demonstration users.
...
Client → token endpoint → bearer header → GraphQL middleware/resolver.
...
Client → token endpoint → bearer header → GraphQL middleware/resolver. Validate both the successful path and the authentication, validation, connectivity, or not-found failure relevant to this example.
...
Use the launch-profile ports. Set `JwtSettings__SecretKey` through environment variables or user secrets; source values are placeholders only.
...
```powershell
dotnet run --project src/Authentication/GraphQL/JwtBearer/Exp.Auth.GraphQLApi.JwtBearer --launch-profile https
```
...
- Run `dotnet build src/Authentication/GraphQL/JwtBearer/Exp.Auth.GraphQLApi.JwtBearer` before manual testing.
