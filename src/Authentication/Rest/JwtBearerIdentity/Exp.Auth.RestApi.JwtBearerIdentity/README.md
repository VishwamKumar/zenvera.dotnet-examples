# REST JWT with ASP.NET Core Identity

## Objective

Combine ASP.NET Core Identity persistence with custom JWT issuance.
...
- Identity user storage with EF Core.
- Registration/login and JWT creation.
- Bearer-protected Weather endpoint.
...
Client → Identity registration/login → JWT service → bearer middleware → protected controller.
...
Client → Identity registration/login → JWT service → bearer middleware → protected controller. Validate both the successful path and the authentication, validation, connectivity, or not-found failure relevant to this example.
...
HTTP 6104; HTTPS 8104. Configure `ConnectionStrings__DefaultConnection` and `JwtSettings__SecretKey` outside source. SQL Server is required at runtime.
...
```powershell
dotnet run --project src/Authentication/Rest/JwtBearerIdentity/Exp.Auth.RestApi.JwtBearerIdentity --launch-profile https
```
...
- Run `dotnet build src/Authentication/Rest/JwtBearerIdentity/Exp.Auth.RestApi.JwtBearerIdentity` before manual testing.
