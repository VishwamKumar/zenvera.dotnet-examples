...
Show a simple REST API-key check and make its security trade-offs visible.
...
- `X-Api-Key` credential transport.
- Middleware-based key validation.
- Weather endpoint protection.
...
Client → API-key middleware → protected controller, or 401 when the key is missing/invalid.
...
Client → API-key middleware → protected controller, or 401 when the key is missing/invalid. Validate both the successful path and the authentication, validation, connectivity, or not-found failure relevant to this example.
...
HTTP 6101; HTTPS 8101. Development keys come from `ApiKeys`; override them with configuration/environment variables and never commit real keys.
...
```powershell
dotnet run --project src/Authentication/Rest/ApiKey/Exp.Auth.RestApi.ApiKey --launch-profile https
```
...
- Run `dotnet build src/Authentication/Rest/ApiKey/Exp.Auth.RestApi.ApiKey` before manual testing.
