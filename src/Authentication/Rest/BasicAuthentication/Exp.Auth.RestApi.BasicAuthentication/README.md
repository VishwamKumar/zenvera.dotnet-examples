...
Demonstrate HTTP Basic credential parsing for comparison, not endorsement.
...
Client → Basic-auth middleware → protected controller, or 401 challenge on failure.
...
Client → Basic-auth middleware → protected controller, or 401 challenge on failure. Validate both the successful path and the authentication, validation, connectivity, or not-found failure relevant to this example.
...
HTTP 6102; HTTPS 8102. Use only development placeholder credentials. Basic authentication requires TLS and is generally not recommended for new public systems.
...
```powershell
dotnet run --project src/Authentication/Rest/BasicAuthentication/Exp.Auth.RestApi.BasicAuthentication --launch-profile https
```
...
- Run `dotnet build src/Authentication/Rest/BasicAuthentication/Exp.Auth.RestApi.BasicAuthentication` before manual testing.
