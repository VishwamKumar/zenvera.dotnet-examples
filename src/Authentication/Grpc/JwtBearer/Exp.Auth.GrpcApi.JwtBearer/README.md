# gRPC JWT bearer authentication

## Objective

Demonstrate JWT issuance and bearer validation for gRPC calls.
...
- JWT issuance through an HTTP helper endpoint.
...
Client → token endpoint → JWT in gRPC metadata → authentication middleware → Weather service.
...
Client → token endpoint → JWT in gRPC metadata → authentication middleware → Weather service. Validate both the successful path and the authentication, validation, connectivity, or not-found failure relevant to this example.
...
Use `Properties/launchSettings.json` for ports. Provide `JwtSettings__SecretKey` through environment variables or user secrets.
...
```powershell
dotnet run --project src/Authentication/Grpc/JwtBearer/Exp.Auth.GrpcApi.JwtBearer --launch-profile https
```
...
- Run `dotnet build src/Authentication/Grpc/JwtBearer/Exp.Auth.GrpcApi.JwtBearer` before manual testing.
