...
Show API-key validation carried in gRPC request metadata.
...
- gRPC metadata credential transport.
- Middleware/interceptor-style validation.
...
gRPC client → metadata key validation → Weather service, or unauthenticated status on failure.
...
gRPC client → metadata key validation → Weather service, or unauthenticated status on failure. Validate both the successful path and the authentication, validation, connectivity, or not-found failure relevant to this example.
...
Use the launch-profile ports documented in `Properties/launchSettings.json`. Replace development keys through configuration; use an HTTP/2-capable client.
...
```powershell
dotnet run --project src/Authentication/Grpc/ApiKey/Exp.Auth.GrpcApi.ApiKey --launch-profile https
```
...
- Run `dotnet build src/Authentication/Grpc/ApiKey/Exp.Auth.GrpcApi.ApiKey` before manual testing.
